...
for target_output in TARGET_VARIABLES:
    ...
    # --- Step 4: Final Model Training on the FULL Dataset ---
    print(f"\n--- Finalizing models for '{target_output}' on ALL available data ---")
    ...
    # Final DKL (trained on all data)
    final_model_custom = DeepGPModel(FULL_X_SCALED, FULL_Y_TORCH, GaussianLikelihood(), FeatureExtractor())
    ...
    for _ in tqdm(range(FINAL_TRAINING_EPOCHS), desc="Final DKL Training on Full Data"):
        opt_c.zero_grad(); loss = -mll_c(final_model_custom(FULL_X_SCALED), FULL_Y_TORCH); loss.backward(); opt_c.step()

    # Final Matern (trained on all data)
    kernel_m = ScaleKernel(MaternKernel(nu=2.5, ard_num_dims=2))
    final_model_matern = ExactGPModel(FULL_X_SCALED, FULL_Y_TORCH, GaussianLikelihood(), kernel_m)
    ...
    for _ in tqdm(range(FINAL_TRAINING_EPOCHS), desc="Final Matern Training on Full Data"):
        opt_m.zero_grad(); loss = -mll_m(final_model_matern(FULL_X_SCALED), FULL_Y_TORCH); loss.backward(); opt_m.step()
